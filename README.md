...
- 扰动需满足：
  ...
  - **通用性强**：对多种 TTS 模型（如 FastSpeech2, VITS, YourTTS）均有效
...
- 关键技术：
  - 利用**梯度优化方法**生成对抗扰动
  - 以多个 TTS 模型的音色重建误差为优化目标，最大化其失败概率
  - 引入感知损失约束，确保扰动在人类听觉系统中不可察觉

#### 3. 不可察觉性保障
- 使用以下指标约束扰动强度：
  - **STOI**（Short-Time Objective Intelligibility）：保证语音可懂度
  - **STFT 距离**：控制频谱变化在感知阈值内
  - **PESQ**：评估语音质量退化程度
- 扰动主要集中在人耳不敏感频段，避免影响主观听感

#### 4. 防御机制
- 加扰后的语音上传或公开后，攻击者即使获取也无法用于：
  - Finetune TTS 模型
  - 提取有效音色嵌入（speaker embedding）
- 因为模型学到的是“被污染”的声学特征，生成语音质量显著下降
...
| 关键技术 | 对抗扰动生成 + 多模型联合优化 + 感知约束（STOI/STFT） |
| 防御对象 | TTS 克隆、Finetune、Voice Conversion |
| 优势 | 鲁棒性强、通用性好、无需事后检测 |
| 局限 | 需在语音发布前处理，无法保护已泄露语音 |

> 🔗 论文发表于：**USENIX Security 2024**  
